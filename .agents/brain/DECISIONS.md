...
- **D15 — FTS5-backed global search with LIKE fallback.** We implemented external-content virtual tables for books, texts, and terms. Substring queries utilize the trigram tokenizer (case-insensitive substring matches) and bm25 ranking. Short queries (< 3 chars) or missing trigram support transparently fall back to the original LIKE search service. Sync triggers automate index updates, and we detect missing index state by checking `<table_fts>_docsize` for backfilling. (Session 2026-06-22, M9.)
